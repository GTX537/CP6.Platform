...
这些 C01 变更目前是待审查、待发布的源码修复，计划使用新的不可变 `0.10.2` 七包集合。下列 P03 历史包和证据不包含这些修复；实际固定包跨仓验收与 C01 关闭仍待完成。真实 CP6 issuer 返回 `max-age=60, must-revalidate`，因此其 60 秒刷新失败边界比 900 秒绝对上限更严格。
