...
"@
    ...
    Assert-True ($moduleText -match "(?s)function Invoke-Cp6P09RuntimeMatrix.+?@\('create','--no-build','receiver','receiver-dapr'\).+?@\('start','receiver','receiver-dapr'\).+?@\('create','--no-build','publisher','publisher-dapr'\).+?@\('start','publisher','publisher-dapr'\)") 'Runtime matrix must create and start receiver before publisher without rebuilding.'
